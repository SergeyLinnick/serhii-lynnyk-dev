...
├── packages/
│   ├── api/                    # Level 6 — API layer (actions, queries)
│   ├── auth/                   # Level 4 — Authentication (better-auth + Drizzle)
│   ├── db/                     # Level 2 — Database (Drizzle ORM, Neon Postgres)
│   ├── eslint-config/          # Level 0 — Shared ESLint v9 flat config
│   ├── models/                 # Level 3 — Zod schemas (drizzle-zod), types, constants
│   ├── prettier-config/        # Level 0 — Shared Prettier configuration
│   ├── typescript-config/      # Level 0 — Shared TypeScript configuration
│   ├── ui/                     # Level 5 — Shared UI components (shadcn/ui)
...
```
Level 7  ─  Apps (web, admin)
Level 6  ─  api        → Server Actions, TanStack Query hooks
Level 5  ─  ui         → shadcn/ui, Radix, Storybook
Level 4  ─  auth       → better-auth + Drizzle adapter
Level 3  ─  models     → drizzle-zod schemas, types, constants
Level 2  ─  db         → Drizzle ORM, Neon Postgres
...
### api (`@workspace/api`) — Level 6

```
packages/api/src/
├── _common/                    # Shared API utilities (auth, query keys)
│   ├── auth.ts                 # Server Action auth helper
│   └── query-keys.ts           # Centralized query key factory
├── context/                    # React providers (ApiProvider)
...
├── middleware.ts               # Proxy helper for route protection
└── types.ts                    # Auth types
```

### models (`@workspace/models`) — Level 3
